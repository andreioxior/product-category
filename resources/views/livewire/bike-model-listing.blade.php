        ...
        <div class="px-4 sm:px-6 lg:px-8 py-8">
            <div class="max-w-7xl mx-auto">
                <div class="flex items-center gap-4 mb-6">
                    @php
                        $breadcrumbs = [
                            ['label' => $modelBike->manufacturer, 'url' => route('bikes.manufacturer', ['manufacturer' => strtolower($modelBike->manufacturer)])],
                            ['label' => $modelBike->model]
                        ];
                    @endphp
                ...
                <div class="mb-8">
                    <flux:heading level="2">
                        {{ $modelBike->manufacturer }} {{ $modelBike->model }} (All Years)
                    </flux:heading>
                ...
                <div class="mb-6 flex items-center gap-4 flex-wrap">
                    ...
                    <flux:button href="{{ route('bikes.manufacturer', ['manufacturer' => strtolower($modelBike->manufacturer)]) }}" variant="subtle">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                        </svg>
                        View All {{ $modelBike->manufacturer }} Models
                    </flux:button>
                </div>

                @isset($availableYears)
                <div class="mb-8 bg-zinc-50 dark:bg-zinc-800 rounded-lg p-6">
                    <flux:heading level="3">Available Years</flux:heading>
                    <div class="grid grid-cols-3 md:grid-cols-6 gap-3 mt-4">
                        @foreach ($availableYears as $year)
                            <flux:button :href="route('bikes.show', ['manufacturer' => strtolower($modelBike->manufacturer), 'model' => str_replace(' ', '-', strtolower($modelBike->model)), 'year' => $year])" variant="subtle" size="sm" class="w-full">
                                {{ $year }}
                            </flux:button>
                        @endforeach
                    </div>
                ...
                @if ($filteredProducts->count() === 0)
                    <div class="text-center py-12">
                        ...
                        <flux:text class="text-zinc-500 dark:text-zinc-400 mb-4">
                            There are no products available for this model.
                        </flux:text>
